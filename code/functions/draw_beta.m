...
%     end
    ...
    Qinv = inv(sse_2 + Q_prmean)+0.1*eye(size(sse_2,1));
    Qinv=tidy_cov_mat(Qinv);
	try
        Qinvdraw = wish(Qinv+eps*eye(size(Qinv)),t-1+Q_prvar);
    catch
        Qinvdraw = wish(1*eye(size(Qinv)),t+Q_prvar);
    end
    
    Qdraw = Qinvdraw\eye(size(Qinvdraw));  % this is a draw from Q